...
impl GpuTDA {
    ...
    pub fn compute_features_gpu(&self, adjacency_cpu: &Array2<bool>) -> Result<CudaSlice<f32>> {
        ...
        let adj_gpu = stream.memcpy_stod(&adj_flat)?;

        // Allocate features on GPU
        let mut features_gpu = stream.alloc_zeros::<f32>(n * 3)?;
    ...
    pub fn count_triangles_gpu(&self, adjacency_cpu: &Array2<bool>) -> Result<usize> {
        ...
        let adj_flat: Vec<bool> = adjacency_cpu.iter().copied().collect();
        let adj_gpu = stream.memcpy_stod(&adj_flat)?;
        let mut count_gpu = stream.alloc_zeros::<i32>(1)?;
        ...
        let result = stream.memcpy_dtov(&count_gpu)?;
    ...
    pub fn compute_betti_0_gpu(&self, adjacency_cpu: &Array2<bool>) -> Result<usize> {
        ...
        let adj_flat: Vec<bool> = adjacency_cpu.iter().copied().collect();
        let adj_gpu = stream.memcpy_stod(&adj_flat)?;
        let mut components_gpu = stream.alloc_zeros::<i32>(n)?;
        ...
        unsafe {
            stream.launch_builder(kernel)
                .arg(&adj_gpu)
                .arg(&mut components_gpu)
                .arg(&(n as i32))
                .launch(cfg)?;
        }

        // Count unique components
        let components = stream.memcpy_dtov(&components_gpu)?;
