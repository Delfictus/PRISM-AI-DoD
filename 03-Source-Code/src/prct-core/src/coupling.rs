...
impl PhysicsCouplingService {
    ...
    pub fn kuramoto_step(
        ...
        if n != natural_frequencies.len() {
            return Err(PRCTError::CouplingFailed("Phase and frequency array size mismatch".into()));
        }
        ...
        for i in 0..n {
            ...
            let dphase_dt = natural_frequencies[i] + (self.coupling_strength / n as f64) * coupling_sum;
    ...
    pub fn calculate_transfer_entropy(
        ...
        if source.len() != target.len() {
            return Err(PRCTError::CouplingFailed("Source and target must have same length".into()));
        }

        if source.len() < lag_steps + 2 {
            return Err(PRCTError::CouplingFailed("Time series too short for lag".into()));
        }
...
mod tests {
    ...
    fn test_kuramoto_synchronization() {
        ...
        // Evolve
        for _ in 0..1000 {
            service.kuramoto_step(&mut phases, &frequencies, 0.01).unwrap();
        }
