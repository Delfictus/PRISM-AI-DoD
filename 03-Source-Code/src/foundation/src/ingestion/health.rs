...
impl HealthReport {
    ...
    pub fn from_stats(stats: IngestionStats, buffer_size: usize, buffer_capacity: usize) -> Self {
        ...
        let ingestion_health = if stats.average_rate_hz > 10.0 {
            ...
            ComponentHealth {
                ...
                message: format!("Low ingestion rate: {:.1} points/sec", stats.average_rate_hz),
                details: HashMap::new(),
            }
        } else {
    ...
    pub fn summary(&self) -> String {
        format!(
            ...
            self.status, self.metrics.active_sources, self.metrics.ingestion_rate, self.metrics.error_rate
        )
    }
}
