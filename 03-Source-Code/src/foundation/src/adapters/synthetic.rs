...
pub enum DataGenerator {
    ...
    SineWave { frequency: f64, amplitude: f64, noise: f64 },
    /// Random walk
...
impl SyntheticDataSource {
    ...
    fn generate_value(&self, time_step: usize, dimension: usize) -> f64 {
        ...
        match &self.generator {
            DataGenerator::SineWave { frequency, amplitude, noise } => {
                let t = time_step as f64 / self.sampling_rate_hz;
                let phase_offset = dimension as f64 * std::f64::consts::PI / 4.0;
                let sine = amplitude * (2.0 * std::f64::consts::PI * frequency * t + phase_offset).sin();
            ...
            DataGenerator::Linear { slope, intercept } => {
                slope * time_step as f64 + intercept
            }
        }
    }
}
