...
impl PhaseCausalMatrixProcessor {
    ...
    pub fn extract_causal_pathways(&self, pcm: &Array2<f64>, threshold: f64) -> Vec<(usize, usize, f64)> {
        let n = pcm.nrows();
        ...
        for i in 0..n {
            ...
            for j in (i + 1)..n {
                if !assigned[j] {
                    ...
                    let phase_diff_wrapped = phase_diff.min(2.0 * std::f64::consts::PI - phase_diff);
...
mod tests {
    ...
    fn test_phase_evolution() {
        ...
        let pcm = Array2::from_shape_fn((3, 3), |(i, j)| {
            if i != j { 0.1 } else { 0.0 }
        });

        let new_phases = processor.evolve_phases(&phases, &frequencies, &pcm, 0.01).unwrap();
