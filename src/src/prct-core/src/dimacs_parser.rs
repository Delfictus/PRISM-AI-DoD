...
pub fn parse_mtx_file<P: AsRef<Path>>(path: P) -> Result<Graph> {
    ...
    for line in reader.lines() {
        ...
        if !header_parsed {
            ...
            if parts.len() >= 3 {
                num_vertices = parts[0].parse()
                    .map_err(|e| PRCTError::InvalidGraph(format!("Invalid vertices in MTX: {}", e)))?;
                num_edges = parts[2].parse()
                    .map_err(|e| PRCTError::InvalidGraph(format!("Invalid edges in MTX: {}", e)))?;
                header_parsed = true;
                println!("[MTX Parser] Graph: {} vertices, {} edges", num_vertices, num_edges);
            }
        } else {
            // Edge line: <vertex1> <vertex2>
            if parts.len() >= 2 {
                let v1: usize = parts[0].parse()
                    .map_err(|e| PRCTError::InvalidGraph(format!("Invalid vertex: {}", e)))?;
                let v2: usize = parts[1].parse()
                    .map_err(|e| PRCTError::InvalidGraph(format!("Invalid vertex: {}", e)))?;
    ...
    if num_vertices == 0 {
        return Err(PRCTError::InvalidGraph("No header found in MTX file".into()));
    }
