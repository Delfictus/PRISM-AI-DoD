...
pub fn phase_guided_tsp(
    ...
    for color in 0..num_colors {
        ...
        let vertices: Vec<usize> = coloring.colors
            .iter()
            .enumerate()
            .filter(|(_, &c)| c == color)
            .map(|(i, _)| i)
...
fn build_phase_guided_tour(
    ...
    let start_vertex = *vertices.iter()
        .min_by(|&&a, &&b| {
            let phase_a = get_vertex_phase(phase_field, a);
            let phase_b = get_vertex_phase(phase_field, b);
            phase_a.partial_cmp(&phase_b).unwrap_or(core::cmp::Ordering::Equal)
        })
        .unwrap();
...
fn get_edge_distance(graph: &Graph, from: usize, to: usize) -> f64 {
    ...
    if let Some((_, _, weight)) = graph.edges.iter()
        .find(|(u, v, _)| (*u == from && *v == to) || (*u == to && *v == from)) {
        return *weight;
    }
