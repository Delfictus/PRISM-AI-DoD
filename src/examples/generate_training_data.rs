...
use prism_ai::data::{GraphGenerator, DatasetExporter};
...
fn main() {
    ...
    assert_eq!(graphs.len(), num_graphs, "Expected {} graphs, got {}", num_graphs, graphs.len());
    ...
    let avg_chromatic = graphs.iter().map(|g| g.chromatic_number).sum::<usize>() as f64 / graphs.len() as f64;

    let avg_difficulty = graphs.iter().map(|g| g.difficulty_score).sum::<f64>() / graphs.len() as f64;

    println!("  Average vertices:        {:.1}", avg_vertices);
    println!("  Average edges:           {:.1}", avg_edges);
    println!("  Average degree:          {:.2}", graphs.iter().map(|g| g.avg_degree).sum::<f64>() / graphs.len() as f64);
    println!("\n  Chromatic numbers:");
